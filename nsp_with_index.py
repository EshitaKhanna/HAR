...
PREDICT_LEN = 5     # predicted steps
INPUT_LEN = 85      # input length
TRAIN_RATE = 0.8      
BATCH_SIZE = 32       
SEED = 42
LEARNING_RATE = 1e-3  
...
# DataLoader preparation with indices
def prepare_data(filtered_data, kept_indices, seed=SEED):

    dataset = Dataset4NSP(filtered_data, kept_indices, input_len=INPUT_LEN, pred_len=PREDICT_LEN)

    # train (80%) and temp (20%)
    train_idx, temp_idx = train_test_split(np.arange(len(dataset)), test_size=0.2, random_state=seed)
    ...
    model_name = os.path.join("lstm_nsp_with_labels", f"lstm_nsp_{INPUT_LEN}_{PREDICT_LEN}")
    np.save(f"{model_name}_data_splits.npy", split_info)
...
def train_model_limu():
    ...
    for i, d in enumerate(filtered_data[:3]):  # first 3 only
        print(f"Sequence {i}: {d.shape}")

    train_loader, val_loader, test_loader, split_info = prepare_data(filtered_data, kept_indices)
    
    # Save test indices
    test_indices = split_info['test_idx']
    np.save(f"{model_name}_test_indices.npy", test_indices)
    print(f"Saved test indices: {test_indices.shape}")
    ...
    return model, preds_array, actuals_array, test_indices, kept_indices, split_info

def train_lstm():
    log_hyperparameters()  

    base_dir = "lstm_nsp_with_labels"
    os.makedirs(base_dir, exist_ok=True)

    model_dir_name = f"lstm_nsp_{INPUT_LEN}_{PREDICT_LEN}"
    model_dir = os.path.join(base_dir, model_dir_name)
    os.makedirs(model_dir, exist_ok=True)

    save_path = os.path.join(model_dir, model_dir_name)
    ...
    train_loader, val_loader, test_loader, split_info = prepare_data(filtered_data, kept_indices, seed=SEED)

    # save test indices 
    test_indices = split_info['test_idx']
    np.save(f"{save_path}_test_indices.npy", test_indices)
    print(f"Saved test indices: {test_indices.shape}")
    ...
    np.save(f"{save_path}_predictions.npy", preds_array)
    np.save(f"{save_path}_actuals.npy", actuals_array)
    print(f"Saved LSTM-NSP predictions: {preds_array.shape}, actuals: {actuals_array.shape}")

    np.save(f"{save_path}_train_losses.npy", np.array(model.train_losses))
    np.save(f"{save_path}_val_losses.npy", np.array(model.val_losses))
    np.save(f"{save_path}_test_losses.npy", np.array(model.test_losses))
    np.save(f"{save_path}_kept_indices.npy", np.array(kept_indices, dtype=object))
    np.save(f"{save_path}_removed_indices.npy", np.array(removed_indices, dtype=object))

    torch.save(model, f"{save_path}.pt")
    print(f"Training completed and saved in {save_path}")
